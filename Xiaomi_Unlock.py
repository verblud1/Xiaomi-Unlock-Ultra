...
def main() -> None:
    ...
    try:
        ...
        resync_done    = False
        dns_check_done = False
        warmup_trigger = target_beijing - timedelta(seconds=35)
        dns_trigger    = target_beijing - timedelta(seconds=40)
        ...
        while True:
            ...
            # DNS-перепроверка за 40s до полуночи — до финальной NTP-калибровки
            if not dns_check_done and now_b >= dns_trigger:
                dns_check_done = True
                try:
                    new_ip = socket.gethostbyname(TARGET_HOST)
                    if new_ip != resolved_ip:
                        warn(f"IP изменился (−40s): {resolved_ip} → {new_ip}")
                        resolved_ip = new_ip
                        ...
                        print(f"\n{col_g}[DNS]   IP стабилен (−40s): {resolved_ip}")
                except Exception:
                    warn("Не удалось проверить DNS за 40s до выстрела")
